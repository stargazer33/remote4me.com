Theme Name: {re}start
Theme URI: http://www.leonart.gr/htmlthemes/restart_v4.2/
Description: Multipurpose HTML Template based on twitter bootstrap. 
Version: 4.2
Author: Leon Turut
Author URI: http://www.leonart.gr




Features
--------

 - Clean, Responsive Design
 - Good typography and large photography
 - Built with {less}
 - Subtle animations & a fresh collapsing header effect
 - Parallax Sections



Credits
--------

 - jQuery v1.9.1 | (c) 2005, 2012 jQuery Foundation, Inc. | jquery.org/license
 - Bootstrap v3.0
 - Font Awesome 4
 - Modernizr 2.6.2 (Custom Build)
 - jQuery FlexSlider v2.0 * http://www.woothemes.com/flexslider/ * Contributing author: Tyler Smith (@mbmufffin)
 - prettyPhoto Lightbox clone for jQuery Author: Stephane Caron (http://www.no-margin-for-errors.com) Version: 3.1.5
 - WOW - v0.1.4 - 2014-02-12 Copyright (c) 2014 Matthieu Aussaguel; Licensed MIT
 - Snap.svg 0.2.0 http://snapsvg.io/ Copyright (c) 2013 Adobe Systems Incorporated. All rights reserved. 
 - Animate.css - http://daneden.me/animate
 - Isotope v1.5.25 jQuery plugin http://isotope.metafizzy.co (Commercial Licence)
 - UItoTop jQuery Plugin 1.2 by Matt Varone
 - Background patterns by Subtle Patterns (http://subtlepatterns.com/)
 - Google Open Sans Webfont



Changelog
---------

 v4.2 [24/05/2015] 
 
- Added Color-Sets functionality
- Added Section Separators functionality (elements.html)

 v4.1 [27/10/2015] 
 
- IE9 slider caption z-index FIX
- Background YouTube Video Sections (elements.html)

 v4.0 [4/05/2015] 
 
- Bootstrap Framework Update to v3.3.4
- Major section redesign to allow full-width sections
- Parallax Sections (elements.html)
- FIX Menu Bugs


 v3.0 [3/08/2014] 
 
- Documentation Page
- Portfolio Redesign
- Typography improvements
- New Service-Teaser Elements
- Clients Section
- Footer Redesign
- Working PHP Ajax Contact Form
- Google Maps API Integration
- Top Toolbar addition
- Section Headers variations


 v2.0 [25/10/2013] 
 
- Twitter Bootstrap v3


 v1.1 [15/04/2013] 

- Sign in & Sign up page 
- Better touch icons
- Small redesign of strict-portfolio page [portfolio.html]
- Added easing.js Jquery Plugin for smoother to-top-button transition [js/easing.js] (Optional)
- Removed outline on a:focus (Thanks to Daniel Collier) 
- Fixed a bug on portfolio resizing [js/restart_theme.js] (Thanks to Liam Carey for reporting & Daniel Collier for fixing it!)
- Assign Z-index to the #toTop element in order not to de hidden on portfolio page (Thanks to Liam Carey)
- Contact: google maps is internationalized (Thanks to Moritz Winter)
- Contact/elements: fixed breadcrumbs (Thanks to Moritz Winter)
- About_us: correct description for class "collapsing_header" (Thanks to Moritz Winter)
- Fixed a bug on "Collapsing Header Effect". When page was loaded with an anchor the header was overlapping the page (Thanks to Moritz Winter)


 v1.0 [05/03/2013] (dd/mm/yyyy)
 
- Initial release



Copyright
----------

 The photos of the live preview are mainly stock images from veer.com
 The "about us" page photo is (CC BY 2.0) (http://creativecommons.org/licenses/by/2.0/deed.en) by HALDANE MARTIN (http://www.flickr.com/photos/haldanemartin/8631197122/)
 The portfolio photos are from http://unsplash.com/ (CC 1.0) http://creativecommons.org/publicdomain/zero/1.0/ and some photos © by Dimitra Giannouka http://www.dimitragiannouka.com

 Apple, the Apple logo and iPhone are trademarks of Apple Inc., registered in the U.S. and other countries.
